    ...
    module Postgres
      class Order::QuantityAverageFilterService < BaseService
        ...
        def average_quantity_gteq
          query.having("(SUM(spree_line_items.quantity)/COUNT(spree_orders.user_id)) >= ?", values)
        end
        ...
        def average_quantity_between
          ...
          query.having("(SUM(spree_line_items.quantity)/COUNT(spree_orders.user_id)) >= ? AND (SUM(spree_line_items.quantity)/COUNT(spree_orders.user_id)) <= ?", values[0], values[1])
        end
